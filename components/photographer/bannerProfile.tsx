...
const BannerProfile: React.FC<PhotoProfileProps> = ({ user }) => {
  ...
  const uploadPhoto = async (file: File) => {
    ...
    const imageUrl = `https://dqqwbvtouglhiutfehvr.supabase.co/storage/v1/object/public/banners/public/${fileName}`;

    const { error: insertError } = await supabase.from('users').update({ banner_url: imageUrl }).eq('id', user?.id);
      ...
      <div className='relative h-80 mb-10'>
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 hover:opacity-100 opacity-0 w-40 h-20'>
              <LuPencil className='text-5xl' />
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{t('photographerspage.changebanner')}</AlertDialogTitle>
              <AlertDialogDescription>{t('photographerspage.uploadbanner')}</AlertDialogDescription>
        ...
        {bannerPic ? (
          ...
          <div className='flex items-center justify-center w-screen h-full bg-gray-300 dark:bg-gray-700'>
