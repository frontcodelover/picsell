...
import { EditCollectionProps } from '@/types/collection';
...
const EditCollection: React.FC<EditCollectionProps> = ({ collectionId, collectionTitle, collectionDescription, onUpdate }) => {
  ...
  const handleEdit = async () => {
    setLoading(true);
    const { error } = await supabase.from('collections').update({ title, description }).eq('id', collectionId);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button className='bg-white text-foreground border hover:bg-primary hover:text-white'>
          <LuPencil />
        </Button>
      </DialogTrigger>
      <DialogContent>
        ...
        <div className='space-y-4'>
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t('collections.collectionname')} disabled={loading} />
          <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t('collections.collectiondescription')} disabled={loading} />
